...
def get_openai_client(api_key=None, model=None):
    ...
    if api_key is None:
        if hasattr(st, 'session_state') and "api_key" in st.session_state:
            ...
            error_msg = "API key OpenAI non fornita e non disponibile in session_state"
            logger.error(error_msg)
            return None
    
    if model is None:
        if hasattr(st, 'session_state') and "model" in st.session_state:
            model = st.session_state.model
        else:
            error_msg = "Modello OpenAI non fornito e non disponibile in session_state"
            logger.error(error_msg)
            return None
    
    if not api_key:
        error_msg = "API key OpenAI vuota o non valida"
        logger.error(error_msg)
        return None
...
def combine_texts_openai(text_direct, text_ocr, openai_model=None, openai_api_key=None):
    """Usa OpenAI per combinare e pulire i testi estratti"""
    # Inizializzazione del logger per i punteggi
    scores_logger = logging.getLogger("SCORES_DEBUG")
    
    # Ottieni i parametri da session_state se non forniti (per retrocompatibilità)
    if openai_model is None:
        if hasattr(st, 'session_state') and "model" in st.session_state:
            openai_model = st.session_state.model
        else:
            logger.error("Modello OpenAI non fornito e non disponibile in session_state per combine_texts_openai")
    ...
    if openai_api_key is None:
        if hasattr(st, 'session_state') and "api_key" in st.session_state:
            ...
            logger.error("API key OpenAI non fornita e non disponibile in session_state per combine_texts_openai")
            return text_direct if len(text_direct) > len(text_ocr) else text_ocr
    ...
    try:
        ...
        # Uso la funzione helper per creare il client OpenAI con supporto DeepSeek
        client = get_openai_client(api_key=openai_api_key, model=openai_model)
        
        if client is None:
            logger.error("Impossibile creare client OpenAI")
            return text_direct if len(text_direct) > len(text_ocr) else text_ocr
        ...
        logger.info(f"Base URL: {client.base_url}")
        logger.info(f"Headers: {client.default_headers}")
        logger.info(f"Modello: {openai_model}")
        logger.info(f"API Key in uso (primi 8 caratteri): {openai_api_key[:8] if openai_api_key else 'None'}...")
...
def combine_texts_ollama(text_direct, text_ocr, ollama_model=None):
    ...
    if ollama_model is None:
        if hasattr(st, 'session_state') and "ollama_model" in st.session_state:
            ollama_model = st.session_state.ollama_model
        else:
            logger.error("Modello Ollama non fornito e non disponibile in session_state per combine_texts_ollama")
            return text_direct if len(text_direct) > len(text_ocr) else text_ocr
    ...
    try:
        ...
        # Se non c'è in cache, chiama l'API
        logger.info(f"Chiamata API Ollama con modello {ollama_model}")
        response = requests.post(
            "http://localhost:11434/api/generate",
            json={
                "model": ollama_model,
...
def analyze_cv_openai(cv_text, job_description, fields, openai_model=None, openai_api_key=None, 
                      evaluation_criteria=None, criteria_weights=None):
    """
    ...
    Args:
        ...
        fields: Campi da estrarre
        openai_model: Modello OpenAI (opzionale, usa st.session_state se non fornito)
        openai_api_key: API key OpenAI (opzionale, usa st.session_state se non fornito)
        evaluation_criteria: Criteri di valutazione (opzionale, usa st.session_state se non fornito)
        criteria_weights: Pesi dei criteri (opzionale, usa st.session_state se non fornito)
    """
    ...
    # Ottieni i parametri da session_state se non forniti (per retrocompatibilità)
    # IMPORTANTE: Nei thread paralleli, st.session_state potrebbe non essere accessibile
    # Quindi se i parametri NON sono forniti, restituiamo errore invece di tentare di accedere a session_state
    if openai_model is None:
        logger.error("Modello OpenAI non fornito come parametro")
        scores_logger.error("Modello OpenAI non fornito come parametro - necessario per thread-safety")
        return None
    
    if openai_api_key is None:
        logger.error("API key OpenAI non fornita come parametro")
        scores_logger.error("API key OpenAI non fornita come parametro - necessario per thread-safety")
        return None
    
    # Usa valori di default se non forniti (non richiedono session_state)
    if evaluation_criteria is None:
        evaluation_criteria = EVALUATION_CRITERIA
        logger.debug("Usando criteri di valutazione predefiniti")
    
    if criteria_weights is None:
        criteria_weights = DEFAULT_CRITERIA_WEIGHTS
        logger.debug("Usando pesi criteri predefiniti")
    
    scores_logger.info(f"Modello: {openai_model}")
    ...
    try:
        ...
        extraction_input_tokens = count_tokens(extraction_prompt, openai_model)
        ...
        if not cached_extraction:
            ...
            if client is None:
                error_msg = "Impossibile creare client OpenAI - get_openai_client ha restituito None"
                logger.error(error_msg)
                scores_logger.error(error_msg)
                return None
            ...
            try:
                ...
                try:
                    ...
                    if json_match:
                        ...
                        error_msg = f"Impossibile analizzare il risultato JSON: {result[:500] if result else 'None'}"
                        logger.error(error_msg)
                        scores_logger.error(error_msg)
                        return None
            except Exception as e:
                error_msg = f"Errore nella chiamata API OpenAI per l'estrazione: {str(e)}"
                logger.error(error_msg)
                import traceback
                logger.error(f"Traceback: {traceback.format_exc()}")
                scores_logger.error(f"{error_msg}\nTraceback: {traceback.format_exc()}")
                return None
        ...
        # Usa i criteri passati come parametro o quelli predefiniti
        criteria_list = evaluation_criteria if evaluation_criteria else EVALUATION_CRITERIA
        
        # Usa i pesi passati come parametro o quelli predefiniti
        criteria_weights_to_use = criteria_weights if criteria_weights else DEFAULT_CRITERIA_WEIGHTS
        ...
        for criteria_id, criteria_label in criteria_list:
            ...
            weight = criteria_weights_to_use.get(criteria_id, 10)
        ...
        evaluation_input_tokens = count_tokens(evaluation_prompt, openai_model)
        ...
        if cached_evaluation:
            ...
            try:
                ...
                evaluation_output_tokens = count_tokens(json.dumps(evaluation_result), openai_model)
            except:
                import re
                ...
                if json_match:
                    ...
                    evaluation_output_tokens = count_tokens(json_match.group(0), openai_model)
                else:
            ...
            try:
                ...
                evaluation_output_tokens = count_tokens(evaluation_text, openai_model)
                ...
                try:
                    ...
                    if json_match:
                        try:
                            ...
                            logger.error(f"Testo problematico: {evaluation_text[:500]}")
                            return None
                    else:
        ...
        # Usa i pesi già definiti sopra (criteria_weights_to_use)
        ...
        error_msg = f"Errore durante l'analisi con OpenAI: {str(e)}"
        logger.error(error_msg)
        import traceback
...
def analyze_cv_ollama(cv_text, job_description, fields, ollama_model=None, 
                      evaluation_criteria=None, criteria_weights=None):
    ...
    # Quindi se i parametri NON sono forniti, restituiamo errore invece di tentare di accedere a session_state
    if ollama_model is None:
        logger.error("Modello Ollama non fornito come parametro")
        scores_logger.error("Modello Ollama non fornito come parametro - necessario per thread-safety")
        return None
    ...
    scores_logger.info(f"Modello: {ollama_model}")
    scores_logger.info(f"Campi da estrarre: {fields}")
    scores_logger.info(f"Job description (troncata): {job_description[:500]}...")
...
def get_results_dir():
    """Crea e restituisce la directory per salvare i risultati incrementali"""
    results_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "analysis_results")
    os.makedirs(results_dir, exist_ok=True)
    return results_dir

def get_current_analysis_file(cv_dir, job_description_hash=None):
    """
    Genera il nome del file per salvare i risultati dell'analisi corrente.
    Usa un hash della directory CV e della job description per identificare univocamente l'analisi.
    """
    results_dir = get_results_dir()
    
    # Crea un hash dalla directory CV e dalla job description
    if job_description_hash is None:
        # Se non fornito, usa solo la directory CV
        analysis_key = f"{cv_dir}"
    else:
        analysis_key = f"{cv_dir}_{job_description_hash}"
    
    analysis_hash = hashlib.md5(analysis_key.encode('utf-8')).hexdigest()[:16]
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"analysis_{analysis_hash}_{timestamp}.json"
    
    return os.path.join(results_dir, filename)

save_results_lock = threading.Lock()

def save_result_incremental(result_entry, analysis_file):
    """
    Salva un singolo risultato nel file di analisi incrementale.
    Aggiunge il risultato al file JSON esistente o lo crea se non esiste.
    L'accesso al file è serializzato per evitare corruzioni in presenza di thread multipli.
    """
        ...
        try:
            ...
            if os.path.exists(analysis_file):
                try:
                    ...
                    logger.warning(f"File di analisi corrotto ({analysis_file}): {decode_err}.Creo un nuovo file e salvo copia di backup.")
                    backup_path = Path(f"{analysis_file}.corrupted_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
                    try:
                        Path(analysis_file).rename(backup_path)
                        logger.warning(f"Backup del file corrotto salvato in {backup_path}")
                    except Exception as rename_error:
                        logger.error(f"Impossibile rinominare il file corrotto {analysis_file}: {rename_error}")
                    saved_results = {
                        "metadata": {
                            "created_at": datetime.now().isoformat(),
                            "analysis_file": analysis_file,
                            "note": "Ricostruito dopo file corrotto"
                        },
                        "results": []
                    }
            else:
            ...
            # Verifica se questo risultato esiste già (per evitare duplicati)
            existing_filenames = [r.get("filename") for r in saved_results.get("results", [])]
            if result_entry.get("filename") not in existing_filenames:
                saved_results["results"].append(result_entry)
                saved_results["metadata"]["last_updated"] = datetime.now().isoformat()
                saved_results["metadata"]["total_results"] = len(saved_results["results"])
                
                # Salva il file
                with open(analysis_file, 'w', encoding='utf-8') as f:
                    json.dump(saved_results, f, ensure_ascii=False, indent=2)
                
                logger.info(f"✅ Risultato salvato incrementale: {result_entry.get('filename')} (totale: {len(saved_results['results'])})")
                return True
            else:
                logger.debug(f"Risultato già presente: {result_entry.get('filename')}")
                return False
        except Exception as e:
            logger.error(f"Errore nel salvataggio incrementale: {str(e)}")
            import traceback
            logger.error(f"Traceback: {traceback.format_exc()}")
            return False

def load_saved_results(analysis_file):
    """
    Carica i risultati salvati da un file di analisi.
    """
    try:
        if os.path.exists(analysis_file):
            with open(analysis_file, 'r', encoding='utf-8') as f:
                saved_data = json.load(f)
            return saved_data.get("results", []), saved_data.get("metadata", {})
        else:
            return [], {}
    except Exception as e:
        logger.error(f"Errore nel caricamento dei risultati salvati: {str(e)}")
        return [], {}

def generate_excel_from_saved_results(analysis_file, fields, output_excel_path=None):
    """
    Genera un file Excel dai risultati salvati su disco.
    Utile per generare un Excel parziale anche se il run si interrompe.
    """
    try:
        results, metadata = load_saved_results(analysis_file)
        
        if not results:
            logger.warning("Nessun risultato trovato nel file salvato")
            return None
        
        logger.info(f"Generazione Excel da {len(results)} risultati salvati")
        
        # Prepara il DataFrame (stesso codice di process_cvs)
        data = []
        for result in results:
            row = {
                "File_PDF": result.get("filename", "sconosciuto"),
                "file_path": result.get("path", "")
            }
            
            # Aggiungi punteggio composito
            ...
            # Aggiungi criteri di valutazione
            evaluation_criteria_to_use = st.session_state.get("evaluation_criteria", EVALUATION_CRITERIA) if 'evaluation_criteria' in st.session_state else []
                ...
                if isinstance(criteria_data, dict):
                    for criteria_id, criteria_label in evaluation_criteria_to_use:
                        if criteria_id in criteria_data:
                            criteria_obj = criteria_data[criteria_id]
                            if isinstance(criteria_obj, dict) and "score" in criteria_obj:
                                row[criteria_label] = int(float(criteria_obj["score"]))
                            else:
        ...
        if output_excel_path is None:
            output_excel_path = analysis_file.replace('.json', '.xlsx')
        ...
        logger.info(f"✅ Excel generato: {output_excel_path} ({len(results_df)} righe)")
        return output_excel_path
        
    except Exception as e:
        logger.error(f"Errore nella generazione Excel dai risultati salvati: {str(e)}")
        import traceback
        logger.error(f"Traceback: {traceback.format_exc()}")
        return None
...
def process_single_cv(pdf_path, job_description, fields, using_openai, idx, total_files, 
                      openai_model=None, openai_api_key=None, ollama_model=None,
                      evaluation_criteria=None, criteria_weights=None, analysis_file=None):
    """
    Processa un singolo CV (funzione helper per il parallelismo)
    
    Args:
        pdf_path: Percorso del file PDF
        job_description: Descrizione del lavoro
        fields: Campi da estrarre
        using_openai: Se True usa OpenAI, altrimenti Ollama
        idx: Indice del CV (per logging)
        total_files: Numero totale di file (per logging)
        openai_model: Modello OpenAI da usare (passato come parametro per thread-safety)
        openai_api_key: API key OpenAI (passato come parametro per thread-safety)
        ollama_model: Modello Ollama da usare (passato come parametro per thread-safety)
        evaluation_criteria: Criteri di valutazione (passato come parametro per thread-safety)
        criteria_weights: Pesi dei criteri (passato come parametro per thread-safety)
        analysis_file: Percorso del file JSON per i salvataggi incrementali
    
    Returns:
        dict con i risultati o None in caso di errore
    """
    filename = os.path.basename(pdf_path)
    logger.info(f"[{idx+1}/{total_files}] Analisi di {filename}...")
    ...
    try:
        ...
        try:
            ...
            # Validazione: verifica che almeno uno dei due testi sia disponibile
            if not text_direct and not text_ocr:
                error_msg = f"❌ Impossibile estrarre testo da {filename} - sia estrazione diretta che OCR hanno fallito"
                logger.error(error_msg)
                return None
        except Exception as e:
            error_msg = f"❌ Errore durante l'estrazione del testo da {filename}: {str(e)}"
            logger.error(error_msg)
            import traceback
            logger.error(f"Traceback: {traceback.format_exc()}")
            return None
        
        # Combina i testi - usa i parametri passati invece di st.session_state
        if using_openai and openai_model:
            try:
                logger.info(f"Combinazione testi con OpenAI per {filename}")
                cv_text = combine_texts_openai(text_direct, text_ocr, openai_model=openai_model, openai_api_key=openai_api_key)
            except Exception as e:
            ...
            try:
                ...
                cv_text = combine_texts_ollama(text_direct, text_ocr, ollama_model=ollama_model)
            except Exception as e:
                logger.error(f"Errore nella combinazione dei testi con Ollama: {str(e)}")
                import traceback
                logger.error(f"Traceback: {traceback.format_exc()}")
        ...
        # Validazione: verifica che cv_text non sia vuoto
        if not cv_text or len(cv_text.strip()) < 10:
            error_msg = f"❌ Testo CV vuoto o troppo corto per {filename} (lunghezza: {len(cv_text) if cv_text else 0})"
            logger.error(error_msg)
            return None
        
        # Analizza il CV - salva temporaneamente i valori necessari in session_state
        logger.info(f"Inizio analisi contenuto CV per {filename} (testo lungo {len(cv_text)} caratteri)")
        result = None
        
        # Verifica che i parametri necessari siano disponibili
        if using_openai:
            if not openai_model:
                logger.error(f"Modello OpenAI non fornito per {filename}")
                return None
            if not openai_api_key:
                logger.error(f"API key OpenAI non fornita per {filename}")
                return None
            logger.info(f"✅ Parametri OpenAI disponibili per {filename}: model={openai_model}, api_key={'impostata' if openai_api_key else 'NON impostata'}")
        else:
            if not ollama_model:
                logger.error(f"Modello Ollama non fornito per {filename}")
                return None
            logger.info(f"✅ Parametri Ollama disponibili per {filename}: model={ollama_model}")
        
        # NON tentiamo più di impostare valori in st.session_state nei thread
        # analyze_cv_openai ora accetta i parametri direttamente, quindi non ne abbiamo bisogno
        
        try:
            if using_openai:
                logger.info(f"🚀 Chiamata analyze_cv_openai per {filename} con modello {openai_model}")
                logger.debug(f"Parametri: model={openai_model}, api_key={'***' if openai_api_key else 'None'}, criteria={len(evaluation_criteria) if evaluation_criteria else 0}")
                result = analyze_cv_openai(
                    cv_text, 
                    job_description, 
                    fields,
                    openai_model=openai_model,
                ...
                if result is None:
                    error_msg = f"⚠️ analyze_cv_openai ha restituito None per {filename}"
                    logger.error(error_msg)
                    logger.error(f"Parametri passati: model={openai_model}, api_key={'impostata' if openai_api_key else 'NON impostata'}")
                    logger.error(f"Lunghezza cv_text: {len(cv_text) if cv_text else 0} caratteri")
                    logger.error(f"Lunghezza job_description: {len(job_description) if job_description else 0} caratteri")
                    logger.error(f"Numero campi: {len(fields) if fields else 0}")
                    import traceback
                    logger.error(f"Stack trace al momento del return None:\n{traceback.format_exc()}")
                    # Suggerisci di controllare i log per dettagli
                    logger.error("💡 Controlla i log in logs/ per dettagli sull'errore specifico")
                else:
                    logger.info(f"✅ analyze_cv_openai completata con successo per {filename}")
            else:
                logger.info(f"🚀 Chiamata analyze_cv_ollama per {filename} con modello {ollama_model}")
                logger.debug(f"Parametri: model={ollama_model}, criteria={len(evaluation_criteria) if evaluation_criteria else 0}")
                result = analyze_cv_ollama(
                    cv_text, 
                    job_description, 
                    fields,
                    ollama_model=ollama_model,
                ...
                if result is None:
                    logger.error(f"⚠️ analyze_cv_ollama ha restituito None per {filename}")
                    logger.error(f"Parametri passati: model={ollama_model}")
                    import traceback
                    logger.error(f"Stack trace al momento del return None:\n{traceback.format_exc()}")
                else:
                    logger.info(f"✅ analyze_cv_ollama completata con successo per {filename}")
        except Exception as e:
            logger.error(f"❌ ECCEZIONE nell'analisi del CV per {filename}: {str(e)}")
            import traceback
            logger.error(f"Traceback completo:\n{traceback.format_exc()}")
            result = None
        ...
        if result:
            ...
            # Salva immediatamente su disco (incrementale)
            # Il file di analisi viene passato come parametro per thread-safety
            if analysis_file:
                save_result_incremental(result_entry, analysis_file)
                logger.info(f"💾 Risultato salvato su disco per {filename}")
            else:
                logger.warning(f"⚠️ analysis_file non fornito, impossibile salvare incrementale per {filename}")
...
def process_cvs(cv_dir, job_description, fields, progress_callback=None):
    ...
    def notify_progress(current, total, message):
        """Aggiorna il progresso solo dal thread principale."""
        if not progress_callback:
            return
        try:
            progress_callback(current, total, message)
        except Exception as progress_error:
            logger.debug(f"Impossibile aggiornare il progresso ({message}): {progress_error}")
    ...
    logger.info(f"Inizio analisi parallela di {total_files} file PDF")
    
    # Crea il file per il salvataggio incrementale dei risultati
    job_description_hash = hashlib.md5(job_description.encode('utf-8')).hexdigest()[:16]
    analysis_file = get_current_analysis_file(cv_dir, job_description_hash)
    st.session_state.current_analysis_file = analysis_file
    logger.info(f"📁 File di analisi incrementale: {analysis_file}")
    st.info(f"💾 Risultati salvati incrementali in: {os.path.basename(analysis_file)}")
    
    # Carica eventuali risultati già salvati (per riprendere un'analisi interrotta)
    saved_results, metadata = load_saved_results(analysis_file)
    if saved_results:
        logger.info(f"📂 Trovati {len(saved_results)} risultati già salvati, riprendo l'analisi")
        st.info(f"📂 Riprendo analisi: {len(saved_results)} CV già analizzati")
    
    completed = len(saved_results)
    if completed:
        notify_progress(completed, total_files, f"Ripresi {completed} risultati salvati")
    
    # Configurazione per il parallelismo: batch grandi per sfruttare meglio la cache di OpenAI
    # Usa un numero elevato di worker per processare molti CV contemporaneamente
    # Questo permette a OpenAI di sfruttare meglio la cache quando ci sono prompt simili
    max_workers = min(50, total_files)  # Massimo 50 worker o il numero di file se minore
    logger.info(f"Utilizzo {max_workers} worker per il processing parallelo")
    st.info(f"🚀 Processing parallelo con {max_workers} worker per sfruttare meglio la cache di OpenAI")
    
    results = []
    
    # Aggiungi i risultati già salvati alla lista (per evitare di riprocessarli)
    saved_filenames = {r.get("filename") for r in saved_results}
    for saved_result in saved_results:
        results.append(saved_result)
    
    # Prepara i parametri da passare ai thread (per thread-safety)
    # Estrai i valori da session_state PRIMA di creare i thread
    openai_model = st.session_state.get('model') if using_openai else None
    openai_api_key = st.session_state.get('api_key') if using_openai else None
    ollama_model = st.session_state.get('ollama_model') if not using_openai else None
    evaluation_criteria = st.session_state.get('evaluation_criteria', EVALUATION_CRITERIA)
    criteria_weights = st.session_state.get('criteria_weights', DEFAULT_CRITERIA_WEIGHTS)
    
    # Validazione dei parametri critici
    if using_openai:
        if not openai_model:
            error_msg = "❌ ERRORE CRITICO: Modello OpenAI non trovato in session_state!"
            logger.error(error_msg)
            st.error(error_msg)
            return [], pd.DataFrame()
        if not openai_api_key:
            error_msg = "❌ ERRORE CRITICO: API key OpenAI non trovata in session_state!"
            logger.error(error_msg)
            st.error(error_msg)
            return [], pd.DataFrame()
        logger.info(f"✅ Parametri OpenAI validati: model={openai_model}, api_key={'impostata' if openai_api_key else 'NON impostata'}")
    else:
        if not ollama_model:
            error_msg = "❌ ERRORE CRITICO: Modello Ollama non trovato in session_state!"
            logger.error(error_msg)
            st.error(error_msg)
            return [], pd.DataFrame()
        logger.info(f"✅ Parametri Ollama validati: model={ollama_model}")
    
    logger.info(f"Parametri per thread: model={openai_model if using_openai else ollama_model}, api_key={'impostata' if openai_api_key else 'non impostata'}")
    
    # Usa ThreadPoolExecutor per processare i CV in parallelo
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Sottometti tutti i task, escludendo quelli già salvati
        future_to_pdf = {
            executor.submit(
                process_single_cv,
                pdf_path,
                ...
                evaluation_criteria,
                criteria_weights,
                analysis_file  # Passa il file di analisi come parametro per thread-safety
            ): (idx, pdf_path)
            for idx, pdf_path in enumerate(pdf_files)
            if os.path.basename(pdf_path) not in saved_filenames  # Salta i file già processati
        }
        ...
        for future in concurrent.futures.as_completed(future_to_pdf):
            ...
            try:
                ...
                completed += 1
                status_message = f"Completato {filename}" if result else f"Errore {filename}"
                notify_progress(completed, total_files, status_message)
                if result:
                    results.append(result)
                    logger.info(f"✅ Completato {filename} ({completed}/{total_files})")
                else:
                    logger.warning(f"⚠️ Analisi fallita per {filename} - result è None")
                    # Log più dettagliato per capire perché è fallita
                    logger.warning(f"   Parametri usati: using_openai={using_openai}, model={openai_model if using_openai else ollama_model}")
                    st.warning(f"⚠️ Non è stato possibile analizzare {filename}")
            except Exception as e:
                logger.error(f"❌ Errore nell'analisi di {filename}: {str(e)}")
                import traceback
                logger.error(f"Traceback: {traceback.format_exc()}")
    ...
    # Ordina i risultati per mantenere l'ordine originale dei file
    results.sort(key=lambda x: pdf_files.index(x["path"]))
    
    # Vecchio loop sequenziale rimosso - ora usiamo il parallelismo sopra
    ...
    # Genera automaticamente l'Excel dai risultati salvati (anche parziale)
    if "current_analysis_file" in st.session_state:
        excel_path = generate_excel_from_saved_results(
            st.session_state.current_analysis_file,
            fields
        )
        if excel_path:
            logger.info(f"📊 Excel generato automaticamente: {excel_path}")
            st.session_state.last_generated_excel = excel_path
            # Non mostriamo il messaggio qui per non interrompere il flusso, ma lo salviamo in session_state
        ...
        with detailed_tab:
            ...
            with cards_tab:
                ...
                if 'results' in st.session_state and st.session_state.results:
                    ...
                    # Crea la griglia di schede usando direttamente i componenti Streamlit
                    if filtered_results:
                        for i, result in enumerate(filtered_results):
                            render_cv_card(result, i)
                    else:
