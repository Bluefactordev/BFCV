...
DEEPSEEK_OPENROUTER_API_KEY = os.getenv("DEEPSEEK_OPENROUTER_API_KEY", "")
...
def refine_missing_fields(cv_text, extraction_result, fields, use_ollama=False, openai_model=None, ollama_model=None, api_key=None):
    ...
    try:
                    ...
                    try:
                        ...
                        if json_match:
                            try:
                                ...
                                st.error("❌ Errore nell'affinamento con OpenAI. La risposta non contiene un JSON valido.")
                                return extraction_result
                        else:
        ...
        st.error(f"❌ Errore generale nell'affinamento dei campi: {str(e)}")
        return extraction_result
...
def analyze_cv_openai(cv_text, job_description, fields):
    ...
    try:
            ...
            try:
                ...
                try:
                    ...
                    if json_match:
                        ...
                        st.error(f"❌ Impossibile analizzare il risultato JSON: {result}")
                        return None
            except Exception as e:
        ...
        - Se un criterio è eccellente, usa un punteggio alto (80+)
        - Se un criterio è scarso, usa un punteggio basso (30-)
        - Ogni criterio può avere un punteggio diverso, non assegnare lo stesso punteggio a tutti i criteri
        ...
        Il punteggio composito è una media pesata dei criteri sopra, usando i pesi indicati per ciascun criterio.
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        st.error(f"❌ Errore nella chiamata Ollama per la valutazione: {response.status_code} - {response.text}")
                        return None
                else:
                    logger.error(f"Errore nella chiamata Ollama per la valutazione: {response.status_code} - {response.text}")
                    st.error(f"❌ Errore nella chiamata Ollama per la valutazione: {response.status_code} - {response.text}")
...
def normalize_dataframe(df):
    ...
    for col in df_normalized.columns:
        # Se è una colonna di punteggio, annota più dettagli
        ...
        try:
            ...
            if using_openai:
                try:
                    ...
                    st.error(f"❌ Errore nell'analisi di {filename} con OpenAI: {str(e)}")
            else:
                try:
                    result = analyze_cv_ollama(cv_text, job_description, fields)
                except Exception as e:
                    logger.error(f"Errore nell'analisi del CV con Ollama: {str(e)}")
                    st.error(f"❌ Errore nell'analisi di {filename} con Ollama: {str(e)}")
...
def show_current_state():
    ...
    current_state = {
        "👤 Profilo": st.session_state.get("current_profile", "Nessuno"),
        "📁 Progetto": st.session_state.get("current_project", "Nessuno"),
        "💼 Posizione": st.session_state.get("current_position", "Nessuna"),
        "📂 Cartella CV": st.session_state.get("cv_dir", "Non selezionata")
    }
...
def render_cv_card(result, idx, show_details=True):
    ...
    if show_details:
        ...
        <div style="margin-bottom:15px;">
            ...
            {" presso " + extraction.get("Università/Istituto", "") if extraction.get("Università/Istituto") else ""}
        </div>
        """
...
def main():
    ...
    with st.sidebar:
        ...
        if "evaluation_criteria" in st.session_state:
            for criteria_id, criteria_label in st.session_state.evaluation_criteria:
                ...
                with col2:
                    # Peso del criterio, default è 10 se non presente
                ...
                with col3:
                    if st.button("❌", key=f"remove_{criteria_id}"):
    ...
    # Se è stata selezionata una directory, verifica la sua esistenza
                ...
                if not pdf_files:
                    ...
                    with st.spinner("Analisi in corso..."):
                        try:
                            ...
                            error_traceback = traceback.format_exc()
                            logger.error(f"Traceback dell'errore:\n{error_traceback}")
                            st.error(f"❌ Errore durante l'analisi dei CV: {str(e)}")
    ...
    if "results" in st.session_state and st.session_state.results:
        ...
        tab_names = ["📊 Panoramica", "🔍 Dettaglio", "⚖️ Confronta", "📋 Schede CV"]
        ...
        with detailed_tab:
            ...
            if 'results' in st.session_state and st.session_state.results:
                ...
                if selected_cv_idx > 0:  # Non è "Seleziona un CV..."
                    ...
                    detail_tabs = st.tabs(["📑 Informazioni Generali", "📊 Valutazione Dettagliata", "📋 CV Completo", "📝 Note"])
            ...
            with cards_tab:
                ...
                if 'results' in st.session_state and st.session_state.results:
                    ...
                    if filter_city_cards:
                        filtered_results = [
                            r for r in filtered_results 
                            if filter_city_cards.lower() in r.get("result", {}).get("extraction", {}).get("Città di residenza", "").lower()
                        ]
